...
function setupClickListener() {
  ...
  alphaPicOpen.addEventListener("click", function () {
    ...
    // Dynamically calculate bottom position based on the height of .property-gallery_col-a minus 7.5rem
    var galleryColA = document.querySelector(".property-gallery_col-a");
    var remInPixels = parseFloat(
      getComputedStyle(document.documentElement).fontSize
    );
    var bottomPositionInPx = galleryColA.offsetHeight - 7.5 * remInPixels; // Convert 7.5rem to pixels and subtract

    // Apply positions for beta and gamma pictures
    document.getElementById("beta-pic-a").style.cssText +=
      "left: 2.5rem; bottom: 2.5rem;";
    document.getElementById(
      "beta-pic-b"
    ).style.cssText = `left: 8.75rem; bottom: ${bottomPositionInPx}px;`;
    document.getElementById("gamma-pic-a").style.cssText +=
      "left: 10rem; bottom: 2.5rem;";
    document.getElementById("gamma-pic-b").style.cssText +=
      "left: 16.25rem; bottom: 2.5rem;";
    document.getElementById(
      "gamma-pic-c"
    ).style.cssText = `left: 22.5rem; bottom: ${bottomPositionInPx}px;`;
  });
}
